...
from gravel_random_2 import gravel_random_adv_train_one_epoch_v2 
...
def process():
    ...
    if configs.evaluate:
        ...
        for pgd_param in configs.ADV.pgd_attack:
            validate_pgd(test_loader, model, criterion, pgd_param[0], pgd_param[1], configs, eval_logger)
        validate(train_loader, model, criterion, configs, eval_logger)
        validate(test_loader, model, criterion, configs, eval_logger)
        return
    ...
    for epoch in range(configs.TRAIN.start_epoch, configs.TRAIN.epochs):
        adjust_learning_rate(configs.TRAIN.lr, optimizer, epoch, configs.ADV.n_repeats, step=60.0)
        ...
        if configs.mode == 1:  # normal train
            train_acc = train_one_epoch(model, train_loader=train_loader, 
                            ...
                            configs=configs,
                            logger=logger)
            train_acc = validate(train_loader, model, criterion, configs, eval_logger)
            test_acc = validate(test_loader, model, criterion, configs, eval_logger)
            ...
            global_noise_data, train_acc = gravel_random_adv_train_one_epoch_v2(model, 
                            train_loader=train_loader, 
                            criterion=criterion, 
                            optimizer=optimizer, 
                            epoch=epoch, 
