...
def get_grad_diff_layer_mask(grad, adv_grad, ratio=0.1):
    ...
    num_ones = int(avg_list.size(0) * ratio)
    num_zeros = avg_list.size(0) - num_ones
    mask_list = [1 for i in range(num_ones)]
    zero_list = [0 for i in range(num_zeros)]
    mask_list.extend(zero_list)
    random.shuffle(mask_list)

    count = 0
    for k,v in layer_mask.items():
        layer_mask[k] = mask_list[count]
        count += 1

    return layer_mask


def gravel_random_adv_train_one_epoch_v2(model, train_loader, criterion, optimizer, epoch, configs, global_noise_data, logger, channel=3, ratio=0.1):  # TODO modify the code
